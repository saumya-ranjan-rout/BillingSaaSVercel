...
interface DataTableProps {
  title: string;
  resource: 'users' | 'tenants' | 'professionals' | 'auditLogs';
...
const convertToCSV = (data: any[], columns: any[]) => {
  ...
  onChange={(e) => {
    console.log("Typing:", e.target.value);  // Log the value as you type
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  {resource !== 'auditLogs' && <TableCell>Actions</TableCell>}
                </TableRow>
              </TableHead>
              <TableBody>
                {paginated.map((row) => (
